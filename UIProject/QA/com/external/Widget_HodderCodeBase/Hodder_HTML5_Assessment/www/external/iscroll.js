 ...
 *  * * * * * * * * * * * * * * * * * * * * * * * * * * * iScroll v4.0 Beta 4 * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * 
 * Copyright (c) 2010 Matteo Spinelli, http://cubiq.org/ Released under MIT
 * license http://cubiq.org/dropbox/mit-license.txt
 * 
...
(function() {
	function iScroll(el, options) {
		...
		// console.log("wrapper", that.wrapper)
		...
		that.options = {
			...
			pullUpLabel : ['Pull up to refresh...', 'Release to refresh...',
					'Loading...'],
			onPullDown : function() {
			},
			onPullUp : function() {
		...
		if (that.pullDownToRefresh) {
			...
			div.innerHTML = '<span class="iScrollPullDownIcon"></span><span class="iScrollPullDownLabel">'
					+ that.options.pullDownLabel[0] + '</span>\n';
		...
		if (that.pullUpToRefresh) {
			...
			div.innerHTML = '<span class="iScrollPullUpIcon"></span><span class="iScrollPullUpLabel">'
					+ that.options.pullUpLabel[0] + '</span>\n';
		...
		that._bind(START_EV);
		/*
		 * that._bind(MOVE_EV); that._bind(END_EV); that._bind(CANCEL_EV);
		 */

		if (hasGesture && that.options.zoom) {
			that._bind('gesturestart');
			that.scroller.style.webkitTransform = that.scroller.style.webkitTransform
					+ ' scale(1)';
		}
	...
	iScroll.prototype = {
		...
		_scrollbar : function(dir) {
			...
			if (!that[dir + 'Scrollbar']) {
				if (that[dir + 'ScrollbarWrapper']) {
					that[dir + 'ScrollbarIndicator'].style.webkitTransform = ''; // Should
																					// free
																					// some
																					// mem
			...
			if (dir == 'h') {
				...
				that.hScrollbarIndicatorSize = m.max(
						m.round(that.hScrollbarSize * that.hScrollbarSize
								/ that.scrollerW), 8);
				...
				that.vScrollbarIndicatorSize = m.max(
						m.round(that.vScrollbarSize * that.vScrollbarSize
								/ that.scrollerH), 8);
		...
		_checkSize : function() {
			...
			scrollerW = m.round(that.scroller.offsetWidth * that.scale), scrollerH = m
					.round((that.scroller.offsetHeight - that.offsetBottom - that.offsetTop)
							* that.scale);
		...
		_pos : function(x, y) {
			...
			that.scroller.style.webkitTransform = trnOpen + that.x + 'px,'
					+ that.y + 'px' + trnClose + ' scale(' + that.scale + ')';
		...
		_start : function(e) {
			...
			if(e.target.nodeName.toLowerCase() == "a" || e.target.nodeName.toLowerCase() == "span" || e.target.nodeName.toLowerCase() == "img" || e.target.nodeName.toLowerCase() == "h3") {
 				e.preventDefault();
				e.stopPropagation();
				//return;
 			}
			if (userAgent.match(/iPhone/i)) {
				...
				/*if (parseInt(imfinity_AppConst.DEVICE_VERSION_NEW.split(".")[0]) > 3) {
					if (e.target.getAttribute("class") != "form-cover" && e.target.nodeName.toLowerCase() != "input" && e.target.nodeName.toLowerCase() != "textarea") {
						e.preventDefault();
					}
					e.stopPropagation();
				} else {*/
				...
				if (e.target.nodeName.toLowerCase() == 'input'
						&& e.target.attributes['readonly'] != undefined) {
					e.preventDefault();
				}
				if (e.target.nodeName.id != "sw-wrapper") {
					// e.preventDefault();
					e.stopPropagation();
				}
			}
			...
			// Registering/unregistering of events is done to preserve resources
			// on Android
			// setTimeout(function () {
		...
		_end : function(e) {
			...
			// console.log("that.startPointX:"+that.startPointX+"
			// that.x:"+point.pageX)

			if (that.swipeRight != null) {
				if (point.pageX > (that.startPointX + 100))
					that.swipeRight();
			}

			if (that.swipeLeft != null) {
				if (that.startPointX > (point.pageX + 100))
					that.swipeLeft();
			}
			...
			if (!that.moved) {
				if (hasTouch) {
					if (that.doubleTapTimer && that.options.zoom) {
						...
						that.zoom(that.pointX, that.pointY, that.scale == 1
										? 2
										: 1);
					} else {
						that.doubleTapTimer = setTimeout(function() {
									...
									ev.initMouseEvent('click', true, true,
											...
											point.clientY, e.ctrlKey, e.altKey,
			...
			if (duration < 300 && that.options.momentum) {
				momentumX = newPosX ? that._momentum(newPosX - that.startX,
						duration, -that.x, that.scrollerW - that.wrapperW
								+ that.x, that.options.bounce
								? that.wrapperW
								: 0) : momentumX;
				momentumY = newPosY
						? that._momentum(newPosY - that.startY, duration,
								-that.y, (that.maxScrollY < 0 ? that.scrollerH
			...
			if (momentumX.dist || momentumY.dist) {
				...
				/*
				 * if (newPosX > 0 || newPosX < that.maxScrollX || newPosY > 0 ||
				 * newPosY < that.maxScrollY) { // Subtle change of scroller
				 * motion that.scroller.style.webkitTransitionTimingFunction =
				 * 'cubic-bezier(0.33,0.66,0.5,1)'; if (that.hScrollbar)
				 * that.hScrollbarIndicator.style.webkitTransitionTimingFunction =
				 * 'cubic-bezier(0.33,0.66,0.5,1)'; if (that.vScrollbar)
				 * that.vScrollbarIndicator.style.webkitTransitionTimingFunction =
				 * 'cubic-bezier(0.33,0.66,0.5,1)'; }
				 */
			...
			if (resetX == that.x && resetY == that.y) {
				...
				if (that.zoomed) {
					if (that.options.onZoomEnd)
						that.options.onZoomEnd.call(that); // Execute custom
															// code on scroll
															// end
					that.zoomed = false;
				}
		...
		_gestChange : function(e) {
			...
			that.scroller.style.webkitTransform = trnOpen + x + 'px,' + y
					+ 'px' + trnClose + ' scale(' + scale + ')';
		...
		refresh : function() {
			...
			if (typeof that.options.snap == 'string') {
				...
				for (i = 0, l = els.length; i < l; i++) {
					...
					that.pagesX[i] = pos.x < that.maxScrollX
							? that.maxScrollX
							: pos.x * that.scale;
					that.pagesY[i] = pos.y < that.maxScrollY
							? that.maxScrollY
							: pos.y * that.scale;
				}
		...
		scrollToElement : function(el, time) {
			...
			pos = that._offset(el);
			pos.x = pos.x > 0 ? 0 : pos.x < that.maxScrollX
					? that.maxScrollX
					: pos.x;
			pos.y = pos.y > 0 ? 0 : pos.y < that.maxScrollY
					? that.maxScrollY
					: pos.y;
			time = time === undefined ? m.max(m.abs(pos.x) * 2, m.abs(pos.y)
							* 2) : time;
		...
		scrollToPage : function(pageX, pageY, time) {
			...
			if (that.options.snap) {
				pageX = pageX == 'next' ? that.currPageX + 1 : pageX == 'prev'
						? that.currPageX - 1
						: pageX;
				pageY = pageY == 'next' ? that.currPageY + 1 : pageY == 'prev'
						? that.currPageY - 1
						: pageY;

				pageX = pageX < 0 ? 0 : pageX > that.pagesX.length - 1
						? that.pagesX.length - 1
						: pageX;
				pageY = pageY < 0 ? 0 : pageY > that.pagesY.length - 1
	...
	var has3d = 'WebKitCSSMatrix' in window && 'm11' in new WebKitCSSMatrix(), hasTouch = 'ontouchstart' in window, hasGesture = 'ongesturestart' in window,
	// hasHashChange = 'onhashchange' in window,
	// hasTransitionEnd = 'onwebkittransitionend' in window,
	hasCompositing = 'WebKitTransitionEvent' in window, isIDevice = (/iphone|ipad/gi)
			.test(navigator.appVersion), isAndroid = (/android/gi)
			.test(navigator.appVersion), RESIZE_EV = 'onorientationchange' in window
			? 'orientationchange'
			: 'resize', START_EV = hasTouch ? 'touchstart' : 'mousedown', MOVE_EV = hasTouch
			? 'touchmove'
			: 'mousemove', END_EV = hasTouch ? 'touchend' : 'mouseup', CANCEL_EV = hasTouch
			? 'touchcancel'
			: 'mouseup', trnOpen = 'translate' + (has3d ? '3d(' : '('), trnClose = has3d
			? ',0)'
			: ')', m = Math;
