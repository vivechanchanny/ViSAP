...
ToolMan._dragsortFactory = {
	 ...
	 */
	makeListSortable : function(list, dragsort, callback) {
		imfinity_utils.log(dragsort);
		ToolMan.appCallback = callback;
	...
	disable : function(list) {
		imfinity_utils.log("list : ");
		list.forEach(function(item){
			ToolMan.events().unregister(item, 'touchstart', null);
			ToolMan.events().unregister(item, 'touchmove', null);
			ToolMan.events().unregister(item, 'touchend', null);
		});
	},

	_onDragStart : function(dragEvent) {
		var id = dragEvent.group.element.id;
		$("#"+id).addClass("dragSelected");
	},
	...
	_onDragEnd : function(dragEvent) {
		...
		ToolMan.appCallback(dragEvent.group.element);
	}
}
