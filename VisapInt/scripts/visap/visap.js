...
(function (ns) {
    ...
    ns.init = function (args) {
        ...
        if ($) {
            ...
            me.cp.createElement();
            if (args) $.extend(me.video, args);
            // Getggr all controls from document.  
            me.readControls();
            //if (!me._misc.SupportsHTML5()) return;
            ...
            //TODO:Need to verify this property 
            ns.mode = args.mode;
    ...
    ///to its list  
    /// </summary>
    //TODO:How does the page determine "isTest", Test or question is part of video conten
    ...
    ns.play = function (_id) {
        ...
        ViTag.debug("Visap:play:Playing the Video for the src:" + ns.CurrentSrc().src);  
         ns.analyticsObj.id=_id;
         ns.analyticsObj.name=ns.currentSrcTypeName ;
        $(document).trigger("VisapLog",[ns.getUserInfo(),"Play",ns.analyticsObj]);        
...
// ns.playVideo=function(src){
//   try
// 	{
//    	me.curSrc.title=src;
//    	ns.currentSrcTypeName="uploaded";
//        me[ns.currentSrcTypeName].play(src);
//         }
//     catch(err)
//    {
//       ViTag.error("Visap:play:Error in play for the id "+err );
//    }
   // };
    ...
    //TODO: This should be in the lists of ns.play method
    ...
    /// </summary>
    /// <param name="t">time video to be played from</param>
    //TODO:This method needs to be named as "pause()"
    ...
    //TODO: Check how is it different than ns.CurrentSrc
    ...
    //TODO:This method should be inline with ns.play - single line
    ...
    //TODO:Need to re-review at later stage
    ...
    ns.RenderCurrentAnnotates = function (annotationObj,ispreview) {
	    try
	    {
		        ...
		        var l = me.ctrl.$videoMContainer.outerWidth(), h = 0, ta = $("#tempAnnotate");
		        //used in Testpage
		        if (ta.length < 1) $("body").append("<div id='tempAnnotate' class='divAnnotate'></div>");
		        ...
		        if (a) {
		            ...
		            ta.html("Annotations: " + a.title + a.description);
		            // Video tag total width
		            h = me.ctrl.$videoMContainer.outerHeight() - 50;
		            if (ta.outerHeight() < h) h = ta.outerHeight();
		           ...
		           me.ctrl.$annotates.animate({ height: a.AnnotationAttributes.height, width: a.AnnotationAttributes.width, left: marginLeft, opacity: "show", top: marginTop }, 1500, function () {
		                $(this).append("<p class='ansContent'>"+unescape(a.title)+"</p>"+ns.htmlDecode(a.description));
		            });
		        }
		        else {
    ...
    try
    {
        ...
        if (whiteboardObj) {
           ...
           // me.ctrl.$WbimgOverlay.attr("width", whiteboardWidth);
           ...
           me.ctrl.$WbimgOverlay.attr("height", txtHeight);
            ...
            if (ns.isFullScreenON) {
                ns.whiteboardFullScreen();//whiteboard will display left by defualt in fullscreen
            }
            else {
   ...
   //To get height of txt content, depending on this hieght imgoverlay height will be set.
    ...
    ns.whiteboardFullScreen = function ()
    {
    try
    {
        if (ns.isFullScreenON) {//If it full screen,whiteboard will display left by defualt.
         ViTag.debug("Visap:whiteboardFullScreen:whiteboard full screen event raised");
            ...
            me.ctrl.$wboardContainer.hide(0).delay(0).toggle('slide', { direction: 'left' },
    ...
    try
    {  
        var vidContainerWidth = 540, hotspotWidth = 15, hotspotHeight = 15, hsInterval, showOnpause = false, isRender = true; //these values are fixed.
        // Video tag total width
        var l = me.ctrl.$videoMContainer.outerWidth(), h = 0;
        //used in Testpage
        ...
        if (hotspotObj) {
         ViTag.debug("Visap:RenderCurrentHotspot:hotspot rendered having tittle,description:"+unescape(hotspotObj.title) +"," + unescape(hotspotObj.description));
          ns.pauseByAction=true;//show's hotspot action and hide the default action container for new theme.
            ...
            // if (ns.TimerHS) clearInterval(ns.TimerHS);
        }
        }
         catch(err)
	    {
     ...
     try
    	{
	        ...
	        me.ctrl.$hotspotCircle.append("<div id='hotspotBox'  class='hotspotBox'> <strong> <span class='hstitle'>" + unescape(hotspotObj.title) + " </span></strong><br/>" + unescape(hotspotObj.description) + "</div>");
    ...
    try
    {
        if (!(document.fullScreen || document.mozFullScreen || document.webkitIsFullScreen || document.msFullscreenElement)) {
            if (!ns.isCustomFullScreen) ns.raiseFullScreen(false);
    ...
    var me = {
        ...
        directURL: {
            ...
            /// </summary>   
            totalDuration: function () {
              //  return me.uploaded.totalDuration();
              if(ns.CurrentSrc)
              return ns.CurrentSrc().videototalduration;
            },
            //This will check the video is in paused state.if its paused it will return true or it will return false.
        ...
        youtube: {
            ...
            /// <returns>Returns total duration</returns> 
            totalDuration: function () {
             //ViTag.debug("Visap:totalDuration:total duration of video"+Math.floor(ns.yt.player.getDuration()));
                //return Math.floor(ns.yt.player.getDuration());
 				if(ns.CurrentSrc)                
                return ns.CurrentSrc().videototalduration;
            },
                  /// <summary>
        ...
        timeline: {
            ...
            /// </summary>  
            currentTime: function () {
                var duration = 0, index = me.bunchSq, time = me[me.timeline.snipType].currentTime(); //time = me.ctrl.video.currentTime;
            ...
            setInterval: function () {
                if (me.cp.interval) clearInterval(me.cp.interval);
        ...
        renderAttrs: function (src) {
          ViTag.debug("Visap:renderAttrs:render tagslinks and other attributes of the video source"+src);
            ...
            me.RenderTags(src);
            me.RenderLinks();
            //me.ctrl.$video.trigger("onSourceChange", me.curSrc);
        ...
        loadData: function (mode, user) {            
            ...
            $.ajax({
                ...
                type: "POST",
                async: false,
                headers: { isStage: true, 'X-Authorization': localStorage.getItem('authT') },
        ...
        //TODO: Remove this method. Use ns.<method> directly
        ...
        bindEvents: function () {
            ...
            me.ctrl.$videoContainer.bind('webkitfullscreenchange mozfullscreenchange fullscreenchange msfullscreenchange', function (e) {
                ns.toggleScreen();
            });
       ...
       cp: {
            ...
            ViTag.debug("Visap:init:Initilization of player");
                ...
                if (!me.cp.ctrl.$play) {
                    ...
                    if (args) $.extend(me.cp.elements, args);
            ...
            createElement: function () {
                if ($("#videoMainContainer").length == 0) {
                    ...
                    $("#tbl2td2").append(me.cp.createEle({ ele: "div", id: "sketchcontainerDefault", attribs: [{ attr: "class", value: "canvascontainer"}] }));
                    ...
                    $("#hotspotCircle").draggable({ containment: "parent", iframeFix: true });
            ...
            setevents: function () {  	         
                ...
                // Play and Pause functionality   
                   ...
                   $("#volume-bar").removeClass("bar").show();
               
                   }  else{
                   $("#volume-bar").addClass("bar").hide();
                ...
                me.cp.ctrl.$play.click(function () {
                    ...
                    console.debug("ddddddddddddd"+me.ctrl.video.ended);
                        ...
                        ns.timelinePlay=false;
                            ...
                            ns.tmVideoEnd = false;
                            if (ns.CurrentSrc().sourcetype == 2) me.cp.clearInterval();
                        }
                    }
                ...
                me.cp.ctrl.$mute.click(function (event) {
                ...
                me.ctrl.video.addEventListener('ended', function () {
                    me.cp.ctrl.play.className = "imgReplay";
                    //me.cp.ctrl.$timelineContainer.show();
                  ...
                  $("body").triggerHandler("onFullScreen", function(placeholdervalue){
                   ...
                   var elem = fsplaceholder, req;
                    ...
                    if (ns.isFullScreenON) {
                        ...
                        // me.cp.ctrl.$FSTags.show();
                        // me.cp.ctrl.$FSlinks.show();
                ...
                $("body").on("onSrcChange", function (event, videoID) {
                    ...
                    setTimeout(function () {
		               $("body").trigger("clearTimer");
                        me.cp.ctrl.seek.value = 0;
                       if (me.cp.time.tmr) clearInterval(me.cp.time.tmr);
                ...
                $("body").on("onTmSrcChange", function () { //when the timeline source changed.
                    ...
                    me.showLoadingimg();
                    me.timeline.setInterval();
                    if (me.cp.time.tmr) clearInterval(me.cp.time.tmr); //this clearinterval is for youtube video.
                });
            ...
            getAudioClass: function (v) {
                ...
                if (typeof v != "undefined") val = v * 100;
                else {
        ...
        tickHandler: function (d,type,typeOfActionList,actionType,ArrayListType) {
            ...
            if (a) {
                if (!allActions.checkObject(a,typeOfActionList)) {
                    ...
                    //TODO:type is not sent later to remove
            ...
            if (a == null)
            actionType.end(null); 
            var endObj = typeOfActionList[0];//CurrentActions.peek(type);
            if (endObj != undefined) {
                if (endObj != null && (d.t >= endObj.endTime || d.t < endObj.listObj.StartTime)) {
      ...
      //if the object found at the respective time ,action will be displayed by calling RenderAnnotate method.
        begin: function (a) {
             ...
             if(pauseOnShow.checkObj(a.StartTime , a.duration))
               return false;            
             me.RenderAnnotate(a);
            //me._misc.debug("Starting at: " + a.StartTime);
        },
        
         //if the object is not found at the respective time,then RenderAnnotate will end by passing null.
        end: function (a) {
        
            //ViTag.info("Visap:annotator:end:ending of annatotaion having title :"+ a.title+" and "+ a.description); 
  ...
  /// <summary>            
    ...
    var hotspot = {
      ...
      init:function(curSrc)
        ...
        tickHandler: function (d) {
            ...
            // ViTag.info("Visap:hotspot:tickHandler:tickhandler of hotspot:"+d.t);  
    ...
    var test = {
        ...
        getNext: function (t) {         
            ...
            if (lst.length > 0) {
                for (i = 0; i < lst.length; i++) {
                    if (Math.floor(lst[i].data.StartTime) > t && found != null) break;
                    if (Math.floor(lst[i].data.StartTime) <= t) found = lst[i];
                }
            }
            return found;
        },
