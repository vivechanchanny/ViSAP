...
(function (ns) {
    ...
    ns.addQuestionData = function (questiondata, callBack) {
     ...
     /// <summary>
    /// Updating aelib questions.
    /// </summary>
     ns.UpdateAelibQues = function(fnPreSend, fnSuccess, fnError) {
       try {
           me.saveActions(JSON.stringify([ns.CurrentSrc()]), fnPreSend, fnSuccess, fnError);
       } catch(err) {
           ViTag.error("Visap.edit:UpdateAelibQues: Error in editing aelib question" + err);
       }
     }
    ...
    ns.onChangeDuration = function (val, id) {
        ...
        internal.updateSeekbar(duration, id);//update video current time(seekbar).
    };
    ...
    ns.timeLineSave = function () {
        ...
        if (ViTag.editTimelineMode) {
            ...
            timelinevideo = { "_id": ViTag.editTmSrcdetials._id, "title": escape(tmLnTitle), "sourcetype": 2, "desc": escape(me.ctrl.$tmDesc.val()), snap: tempData[snippetIndex].data.snap, "src": tempData, "videototalduration": Math.round(timelineduration), "category": [""] };
        }
        else {
            ...
            timelinevideo = { "title": escape(tmLnTitle), "sourcetype": 2, "desc": escape(me.ctrl.$tmDesc.val()), snap: tempData[snippetIndex].data.snap, "src": tempData, "videototalduration": Math.round(timelineduration), "category": [""] };
        }
                    ...
                    ns.yt.player.addEventListener("onStateChange", function (e) {
                        ...
                        switch (ns.yt.videoState) {
                            ...
                            case YT.PlayerState.PAUSED:
                                ...
                                //Enable edit panel only when user has paused the video
                                if(ns.videoPaused){
                                    ns.enableEditPanel();
                                }
                                break;
        ...
        calldelete: function (ID, fnPreSend, fnSuccess, fnError) {
            $.ajax({
                ...
                error: function (err) {
                    ...
                    if (err.status === 400) {
                        $.confirm(err.statusText).then(function () { });
                        return;
                    }
        ...
        timeline: {
            ...
            editSnapshots: function (tmSrcBunch) {
                ...
                for (var i = 0; i < tmSrcBunch.length; i++) {
                    ...
                    div.appendChild(timeLabel);
                    div.appendChild(img);
                    me.ctrl.$startcapture.append(div);
    ...
    ns.DisplayingWhiteboard = function () {
        try {
            var direction = $('input[name="direction"]:checked').val(); //getting position radio button value.
